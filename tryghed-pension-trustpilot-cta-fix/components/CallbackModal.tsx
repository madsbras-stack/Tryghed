...
export default function CallbackModal(){
  ...
  async function submit(e: React.FormEvent){
    ...
    setStatus("sending");
    try{
      const res = await fetch("/api/callback", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name, phone })});
  ...
  return (
    <>
      <button id="ring-mig-op" onClick={()=>setOpen(true)} className="fixed md:hidden bottom-4 right-4 btn btn-primary z-40">
        📞 Ring mig op
      </button>
      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-white rounded-2xl p-6 w-[90%] max-w-md shadow-xl">
            ...
            {status==="ok" ? (
              <div className="mt-4 p-4 rounded-lg bg-green-50 text-green-800">Tak! Vi ringer til dig inden for 24 timer.</div>
            ) : (
              <form onSubmit={submit} className="mt-4 grid gap-3">
                ...
                <label className="block">
                  ...
                  <input type="tel" className="input w-full" value={phone} onChange={(e)=>setPhone(e.target.value)} required />
                </label>
                <div className="flex gap-3 mt-2">
                  <button type="submit" className="btn btn-primary" disabled={status==="sending"}>{status==="sending" ? "Sender..." : "Send"}</button>
